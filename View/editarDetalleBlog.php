<div class="container">
    <fieldset>
        ...
        <form action="/index.php?url=registroBlogControler" enctype="multipart/form-data" method="post" name="formBlog">
            <label for="nombre">Titulo del Blog: </label><input type="text" name="nombre" id="nombre" value="<?= $blog->getTitulo()?>" placeholder="Titulo del Blog"><br/>
            <label for="link">Enlace al Blog: </label><input type="text" name="link" id="link" value="<?= $blog->getLink()?>" placeholder="Escribe la dirección completa del blog"><br/>
            <label for="autor">Autor del Blog: </label><input type="text" name="autor" id="autor" value="<?= $blog->getAutor()?>" placeholder="Autor del blog"><br/>
            <label for="img">Sube una Imagen: </label><input type="file" name="img" id="img"><br/>
            <label for="descripcion">Descripción: </label><textarea name="descripcion" id="descripcion" cols="30" rows="10" placeholder="Descripción del blog"><?= $blog->getDescripcion()?></textarea><br/>
